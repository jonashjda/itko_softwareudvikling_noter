...
function nyUgeplan() {
  ...
  ugePlaner.splice(aktuelUgeIndex + 1, 0, ugeplan);
  console.log(ugePlaner);
  aktuelUgeIndex += 1;
...
function gemEksisterendeUgeplan() {
  ...
  ugePlaner[aktuelUgeIndex].mandag = mandagNy;
  ugePlaner[aktuelUgeIndex].tirsdag = tirsdagNy;
  ugePlaner[aktuelUgeIndex].onsdag = onsdagNy;
  ugePlaner[aktuelUgeIndex].torsdag = torsdagNy;
  ugePlaner[aktuelUgeIndex].fredag = fredagNy;
  ugePlaner[aktuelUgeIndex].lørdag = lørdagNy;
  ugePlaner[aktuelUgeIndex].søndag = søndagNy;
...
function sletUgeplan() {
  ugePlaner.splice(aktuelUgeIndex, 1);
    console.log(ugePlaner);
    if (ugePlaner.length == 0){
      textUgeNummer.value("");
      textMandag.value("");
      textTirsdag.value("");
      textOnsdag.value("");
      ...
      textSøndag.value("")
      aktuelUgeIndex -= 1;
    } else if (aktuelUgeIndex == ugePlaner.length){
      forrigeUge();
...
function visUgeplan(index) {
  textUgeNummer.value(ugePlaner[index].ugeNummer);
  textMandag.value(ugePlaner[index].mandag);
  textTirsdag.value(ugePlaner[index].tirsdag);
  textOnsdag.value(ugePlaner[index].onsdag);
  textTorsdag.value(ugePlaner[index].torsdag);
  textFredag.value(ugePlaner[index].fredag);
  textLørdag.value(ugePlaner[index].lørdag);
  textSøndag.value(ugePlaner[index].søndag);  
}
...
function næsteUge() {
  if (ugePlaner.length == 0){
    ...
    aktuelUgeIndex += 1;
    if (aktuelUgeIndex == ugePlaner.length){
